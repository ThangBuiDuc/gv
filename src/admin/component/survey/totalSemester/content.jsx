...
const Content = ({ action }) => {
  ...
  const csv = useQuery({
    queryKey: ["TOTAL_CSV_CTGD", { type: action.value }],
    queryFn: async () => {
  ...
  const csv1 = useQuery({
    queryKey: ["TOTAL_CSV1_CTGD", { type: action.value }],
    queryFn: async () => {
      return await fetch(`${import.meta.env.VITE_OVERALL_SURVEY}`, {
        method: "POST",
        body: JSON.stringify({
  ...
  const camthi = useQuery({
    queryKey: ["TOTAL_CAMTHI_CTGD", { type: action.value }],
    queryFn: async () => {
      return await fetch(
        `${import.meta.env.VITE_EDUMNG_DS_CAM_THI}${action.hocky}/${
          action.manamhoc
        }`,
        {
          headers: {
  ...
  if (csv.isLoading && csv.isFetching) {
    return (
      <ReactLoading
        type="spin"
        color="#0083C2"
  ...
  if (Array.isArray(csv.data) && csv.data.length === 0) {
    <div className="flex justify-center">
