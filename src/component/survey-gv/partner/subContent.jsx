...
export default function Index({ toggle, setToggle, dataSent,afterUpdate,setAfterUpdate }) {
  const { subject_code, class_code, present } = dataSent;
    ...
    Swal.fire({
      ...
      preConfirm: async () => {
        ...
        if (result === 200) {
          ...
          setToggle(!toggle)
          Swal.fire({ title: "Đánh giá dự giờ thành công!", icon: "success" });
        } else
          Swal.fire({
  ...
  return preData ? (
    <div className="flex flex-col pl-[20px] gap-[20px] ">
      <label onClick={() => setToggle(!toggle)} className="w-fit cursor-pointer">
        <BiArrowBack size={"40"} />
      </label>
